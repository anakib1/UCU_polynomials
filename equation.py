...
class Polynomial:
    def __init__(s, coefs):
        ...
        s.n = len(xd)
    def __str__(s):
        return f"Polynomial(coeffs={s.c})"
    def degree(s):
        return s.n - 1
    def coeff(s, i):
        return s.c[s.n - 1 - i]
    def evalAt(s, x):
        return sum([pow(x, s.n - 1 - i) * s.c[i] for  i in range(s.n)])
    def __eq__(s, o):
        if type(o) == int:
            return s.n == 1 and s.c[0] == o
        return type(o) == type(s) and s.c == o.c
    def __ne__(s, o):
        return not s == o
    def __hash__(s):
        return s.evalAt(17)
    def scaled(s, k):
        return Polynomial([x * k for x in s.c])
    def derivative(s):
        return Polynomial([(s.n - i - 1) * s.c[i] for i in range(s.n - 1)])
    def addPolynomial(s, o):
        return Polynomial(list(map(lambda x : x[0] + x[1], zip([0] * (max(s.n, o.n) - s.n) + s.c, [0] * (max(s.n, o.n) - o.n) + o.c)))) if type(o) == type(s) else None
    def multiplyPolynomial(s, o):
        x = [0] * (s.n + o.n - 1)
        for i in range(s.n):
            for j in range(o.n):
                x[i + j] += s.c[i] * o.c[j]
...
class Quadratic(Polynomial):
    def __init__(s, coefs):
        ...
        super().__init__(xd)
    def __str__(s):
        return f'Quadratic(a={s.c[0]}, b={s.c[1]}, c={s.c[2]})'
    def discriminant(s):
        return s.c[1] ** 2 - 4 * s.c[0] * s.c[2]
    def numberOfRealRoots(s):
        d = s.discriminant()
        if d < 0:
            return 0
        elif d == 0:
    ...
    def getRealRoots(s):
        ...
        if d < 0:
            ...
            return [-s.c[1] / (2 * s.c[0])]
        return [(-s.c[1] - math.sqrt(d))/(2 * s.c[0]), (-s.c[1] + math.sqrt(d))/(2 * s.c[0])]
